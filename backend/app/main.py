...
def process_images() -> None:
    ...
    image_processing.image_resizing(
        config.WORKING_DIR_DDS_TO_PNG, config.WORKING_DIR_DDS_4K, "PNG", 2.0, "SINC"
    )
    image_processing.image_resizing(
        config.WORKING_DIR_DDS_4K, config.WORKING_DIR_DDS_2K, "PNG", 0.6667, "SINC"
    )

    # Resize TGA assets.
    image_processing.image_resizing(
        config.WORKING_DIR_TGA_TO_PNG, config.WORKING_DIR_TGA_4K, "PNG", 2.0, "SINC"
    )
    image_processing.image_resizing(
        config.WORKING_DIR_TGA_4K, config.WORKING_DIR_TGA_2K, "PNG", 0.6667, "SINC"
    )
...
def text_positional_value_comparison() -> None:
    ...
    original_text.calculate_scaling_factors(
