...
"""
This module provides functionality for scaling positional values in text files.

It utilizes a database to store scaling factors and applies them
to individual properties within files.
"""
...
def apply_scaling_factors(
    content: str, original_values: dict[str, list[float]], resolution: str
) -> str:
    """
    Applies scaling factors to the content of a text file.

    This function searches for a property within a string and replaces its value
    with the scaled value, unless the original value is '-1', contains '%', '@', or '10s'.
    ...
    try:
        with Session(engine) as session:
            # Prefetch scaling factors for all properties to minimize database calls.
        ...
        log.exception(
            "An unexpected error occurred while applying scaling factors.", exc_info=error
        )
...
def calculate_property_scaling(
    ...
    for prop in original.items():
        ...
        if prop in scaled and len(original[prop]) == len(scaled[prop]):
            ...
            factors = [
                s / o for o, s in zip(original[prop], scaled[prop], strict=True) if o != 0
            ]
    ...
    if value.startswith("{"):
        return f"{prop} = " + re.sub(
            ...
            lambda m: f"{m.group(1)} = {
                m.group(2) if m.group(2) == "-1"
                or any(x in m.group(2) for x in ["%", "@", "10s"])
...
def scale_positional_values_worker(args: tuple[Path, Path, Path, str]) -> str:
    """
    ...
    Args:
    ----
    - input_directory (Path): The directory containing the file to process.
    - output_directory (Path): The directory to output processed files.
    - input_file (Path): The path to the text file.
    - resolution (str): The target resolution (e.g., "2K", "4K").
...
def scale_positional_values(
    ...
    This function reads text files from the input directory, applies a
    scaling factor to specific positional attributes, and writes the
    modified content to the output directory.
...
def calculate_scaling_factors(
    ...
    """
    Calculates scaling factors for text files by comparison with 2K and 4K counterparts.

    This function reads original text files, their 2K and 4K scaled versions,
    and calculates scaling factors for individual properties and overall scaling
    factors for each resolution.
